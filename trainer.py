...
#                 meter.update(targets[:,:,:-pad_h,:-pad_w], outputs[:,:,:-pad_h,:-pad_w])
        ...
        dice, acc = epoch_log(phase, epoch, epoch_loss, meter, start)
    ...
    def train_end(self):
        ...
        df_data=np.array([train_loss,train_dice,train_iou, train_acc, val_loss,val_dice,val_iou, val_acc]).T
        df = pd.DataFrame(df_data,columns = ['train_loss','train_dice', 'train_acc', 'val_loss','val_dice', 'val_acc'])
    ...
    def fit(self, epochs):
        ...
        for epoch in range(self.num_epochs-epochs, self.num_epochs):
            ...
            if val_acc > self.best_val_acc:
                ...
                os.makedirs('models_acc/', exist_ok=True)
                torch.save(state, 'models_acc/'+self.name+'_best_acc.pth')
            print()
            self.train_end()